...
#' @details XXXXX
#' 
#' Default values of \code{twinspan} (e.g. definition of pseudospecies cut levels, number of hierarchical divisions etc.) are the same as the default values of the original TWINSPAN program (and also WinTWINS).
#' 
#' When calculating modified TWINSPAN (\code{modif = TRUE}), one needs to choose number of target clusters (argument \code{cluster}) instead of hierarchical levels of divisions as in standard TWINSPAN (argument \code{levels}), and also measure of dissimilarity (\code{diss}) to calculate heterogeneity of clusters at each step of division (the most heterogeneous one is divided in the next step). Four types of beta diversity measures are currently implemented:
#' \itemize{
...
#' }
#' @references \itemize{
#' \item Baselga A., Jimenez-Valverde A. & Niccolini G. (2007): A multiple-site similarity measure independent of richness. Biology Letters, 3: 642-645.
...
#' \item Whittaker R.H. (1960): Vegetation of the Siskiyou mountains, Oregon and California. Ecological Monographs, 30:279-338.
#' }
#' @examples
